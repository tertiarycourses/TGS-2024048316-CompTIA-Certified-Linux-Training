...
def _api_key():
    """Same resolution order as lms_push.py: env, project key file, user key file."""
    import os
    for name in ("LMS_TMS_API_KEY", "EXTERNAL_API_KEY_FOR_CLAWDBOT"):
        v = os.environ.get(name, "").strip()
        if v:
            return v
    for path in (os.path.join(os.getcwd(), ".claude", "lms_tms_api_key"),
                 os.path.join(os.path.expanduser("~"), ".claude", "lms_tms_api_key")):
        try:
...
def _headers(url):
    """x-api-key goes ONLY to the LMS-TMS host — never to Google export URLs."""
    if _KEY and url.startswith(API):
        return {**UA, "x-api-key": _KEY}
    return UA
